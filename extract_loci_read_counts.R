...
# count loci
loi <- readRDS("../CNV_NONORM_for_manuscript/loci_of_interest.RDS")

library(dplyr)
library(purrr)

# Convert the list to a data frame for easier lookup
loi_df <- map_dfr(names(loi), function(key) {
  tibble(locus = loi[[key]], element = key)
})

sums <- combined_df_merged %>%
  group_by(NIDA2, locus) %>%
  summarise(locus_sum = sum(reads))

# Join the data frames based on locus
sums_replaced <- sums %>%
  left_join(loi_df, by = "locus") %>%
  mutate(locus = coalesce(element, locus)) %>% # Replace locus with element if available
  select(-element) # Remove the element column

length(unique(sums_replaced$NIDA2))


pm <- sums_replaced[grepl("PM2", sums_replaced$locus),]

pm_sums <- pm %>%
  group_by(NIDA2, locus) %>%
  summarise(locus_sum = sum(locus_sum))

library(tidyr)
library(tibble)

# Pivot the data to make locus as columns and NIDA2 as rows
pm_wide <- pm_sums %>%
  pivot_wider(names_from = locus, values_from = locus_sum) %>%
  column_to_rownames(var = "NIDA2")

# Replace NA with 0
pm_wide[is.na(pm_wide)] <- 0

pm_wide

#select only relevant nida
data_2021 <- readRDS("combined_df_merged_2021_only.RDS")
data_2022 <- readRDS("combined_df_merged_2022_only.RDS")

data_all <- rbind(data_2021, data_2022)

nida_used_in_study <- unique(data_all$NIDA2)

pm_final <- pm_wide[rownames(pm_wide) %in% nida_used_in_study,]


write.csv(pm_final, "PM2_read_counts.csv")
...
# Replace NA with 0
sums_wide[is.na(sums_wide)] <- 0

sums_final <- sums_wide[rownames(sums_wide) %in% nida_used_in_study,]
sums_final_pm2 <- sums_final[,colnames(sums_final) %in% loi$PM2]

write.csv(sums_final, "sums_amps_ALL.csv", row.names = T)
write.csv(sums_final_pm2, "sums_amps_PM2.csv", row.names = T)
